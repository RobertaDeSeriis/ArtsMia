...
public class ArtsmiaDAO {
	...
	//METODO 1 SE HO MENO DI 30 ARCHI, ALTRIMENTI TROPPO LENTO, 
...
*/
	...
	public List<CoppiaId> getCopppiaId(Map<Integer, ArtObject> idMap) {
		String sql= "SELECT e1.object_id, e2.object_id,COUNT(*) AS peso "
				+ "FROM exhibition_objects e1, exhibition_objects e2 "
				+ "WHERE e1.exhibition_id = e2.exhibition_id AND e1.object_id > e2.object_id  "
				+ "GROUP BY  e1.object_id, e2.object_id ";
		...
		try {
			...
			while(res.next()) {
				result.add(new CoppiaId (idMap.get(res.getInt("e1")), idMap.get(res.getInt("e2")), res.getInt("peso")));
			}
			conn.close();
			return result; 
		}
